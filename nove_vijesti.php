...
<body onload="SakrijSubmenu()" class="body_vijesti">
	...
	<section id="frejm">
		<?php
			...
			$vijesti = array();
			$lista_fajlova = scandir("phpskripte/novosti/nove_vijesti");
			$lista_vijesti = array();
			for ($i = 0; $i < count($lista_fajlova); $i++) {
				if (!is_dir('phpskripte/novosti/nove_vijesti/' . $lista_fajlova[$i])) {
					$sadrzaj_fajla = file('phpskripte/novosti/nove_vijesti/' . $lista_fajlova[$i]);
			...
			foreach ($vijesti as $news) {
				$sadrzaj_stranice = '<br><div>';
					...
					if ($brojac != count($news)) {	
						$ime_detalji = 'phpskripte/novosti/nove_vijesti/detalji/' .
